...
import { useGetItemQuery } from '../../../hooks/useGetItemQuery.tsx';
...
const url = 'http://127.0.0.1:8000/api/settings/get-setting';

export default function TanStackTable() {
  const user = useUserStore((state) => state.user);
  const { payload } = useGetItemQuery(
    queryFunction,
    url,
    ['settings'],
    user?.token,
  ...
  const virtualList = [];
  virtualList.push(payload ? payload : {});
